...
12. [☁️ CI/CD y Despliegue](#12--cicd-y-despliegue)
...
El microservicio de **Feed y Búsqueda (M06)** tiene como objetivo ser el punto central de descubrimiento de parches dentro de la plataforma PATRIC.IA. Este servicio construye y expone un feed personalizado de parches activos y públicos ordenados por relevancia, permite la búsqueda y filtrado en tiempo real con respuesta inferior a 1 segundo (RNF01), ofrece recomendaciones basadas en intereses del usuario, y gestiona la acción de unirse a un parche público directamente desde el feed (RF09 parcial). Opera sobre el puerto `8081` y se integra con Redis para caché distribuida y PostgreSQL como base de datos principal.
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>Feed Personalizado (RF17)</strong></td>
    ...
    <tr>
      <td><strong>Búsqueda y Filtrado (RF18)</strong></td>
    ...
    <tr>
      <td><strong>Recomendaciones (RF19)</strong></td>
    ...
    <tr>
      <td><strong>Unirse a un Parche (RF09 parcial)</strong></td>
...
#### `release/*`
- **Propósito:** Congelar cambios para estabilizar pruebas previas al deploy.
- **Base:** `develop`. Merge a `main` (tag `vX.Y.Z`) y de vuelta a `develop`.

#### `hotfix/*`
- **Propósito:** Corregir un bug crítico detectado en `main`.
- **Base:** `main`. Merge a `main` (tag PATCH) y a `develop`.
...
**Reglas:** camelCase o descriptivo, máximo 50 caracteres.
...
```
**Ejemplos:**
- `hotfix/corregirScoreFeed`
- `hotfix/fixBusquedaFiltros`
...
### 1️⃣ Feed Personalizado de Parches (RF17)
...
### 2️⃣ Registrar Interacción con un Parche
...
### 3️⃣ Búsqueda y Filtrado de Parches (RF18)
...
### 4️⃣ Recomendaciones de Parches (RF19)
...
### 5️⃣ Unirse a un Parche desde el Feed (RF09 parcial)
...
| ⚠️ **Excepción** | 🔢 **HTTP** | 💬 **Error Code** | 📝 **Escenario** |
|:----------------|:----------:|:-----------------|:----------------|
...
│   │   │   │   ├── 📁 dto/
│   │   │   │   │   ├── 📁 request/
│   │   │   │   │   │   ├── InteractRequest.java             (action: VIEW | JOIN | SKIP)
│   │   │   │   │   │   └── SearchRequest.java               (q, category, campusZone, status, dateFrom, dateTo, maxGroupSize, hasAvailableSpots)
│   │   │   │   │   └── 📁 response/
│   │   │   │   │       ├── PatchSummaryResponse.java        (datos del parche + affinityScore + isMember)
│   │   │   │   │       ├── PatchRecommendationResponse.java (patchId + patch + affinityScore + reason)
│   │   │   │   │       └── SearchResponse.java              (content + paginación)
│   │   │   │   ├── 📁 mapper/
│   │   │   │   │   └── PatchDomainMapper.java
│   │   │   │   └── 📁 service/
│   │   │   │       ├── FeedService.java                     (implementa FeedUseCase)
│   │   │   │       ├── JoinPatchService.java                (implementa JoinPatchUseCase)
│   │   │   │       ├── RecommendationService.java           (implementa GetRecommendationsPort)
│   │   │   │       └── RegisterInteractionService.java      (implementa RegisterInteractionPort)
│   │   │   │   └── 📁 usecase/
│   │   │   │       └── SearchPatchesUseCaseImpl.java        (implementa SearchPatchesUseCase)
│   │   │   │
...
│   │   │   │   │   ├── UserInterest.java
│   │   │   │   │   └── 📁 enums/
│   │   │   │   │       ├── CampusZone.java                  (BIBLIOTECA, CAFETERIA, CANCHA, SALON, PARQUEADERO, EXTERNO)
│   │   │   │   │       ├── InteractionType.java             (VIEW, JOIN, SKIP)
│   │   │   │   │       ├── MembershipStatus.java
│   │   │   │   │       ├── PatchCategory.java               (STUDY, SPORTS, CULTURE, GAMING, FOOD, OTHER)
│   │   │   │   │       └── PatchStatus.java
│   │   │   │   ├── 📁 ports/
│   │   │   │   │   ├── 📁 in/
│   │   │   │   │   │   ├── FeedUseCase.java
...
│   │   │       │   └── 📁 persistence/
│   │   │       │       ├── PatchSpecification.java           (filtros dinámicos JPA Criteria)
│   │   │       │       ├── 📁 entity/
│   │   │       │       │   ├── PatchEntity.java
